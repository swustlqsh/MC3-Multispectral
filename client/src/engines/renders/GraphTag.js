...
class GraphTag extends Render {
  ...
  showImage (imageIndex) {
    ...
    this._via_current_image.onload = function () {
      ...
      this._via_canvas_height_scale = this._via_current_image.naturalHeight / this._via_canvas_width
  ...
  zoom_in () {
    ...
    if (this._via_canvas_zoom_level_index === (VIA_CANVAS_ZOOM_LEVELS.length - 1)) {
    } else {
      this._via_canvas_zoom_level_index += 1
      this._via_is_canvas_zoomed = true
      let zoom_scale = VIA_CANVAS_ZOOM_LEVELS[ this._via_canvas_zoom_level_index ]
